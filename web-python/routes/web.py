...
def init():
    ...
    get_receipt_by_customer = cassandra_helper.session.prepare("select customer_name,zip,receipt_id,credit_card_number from receipts_by_customer_zip WHERE customer_name=? limit 300")
...
@web_api.route('/test')
def index1():
    return render_template('receipts_by_product_title.jinja2')

@web_api.route('/search1')
def search1():
    search_term = request.args.get('s')
    ...
    query = "SELECT receipt_id,product_id,product_name FROM receipts WHERE solr_query = '{%s}' LIMIT 300" % solr_query
...
@web_api.route('/fraud')
def fraud():
    query = "select * from fraud_detections limit 300"

    results = cassandra_helper.session.execute(query)

    return render_template('fraud.jinja2',frauds = results)

@web_api.route('/receipts_search')
def receipts_search():
        global get_receipt_by_customer
        results = None

        cc_no = request.args.get('name')

        if cc_no:
            results= cassandra_helper.session.execute(get_receipt_by_customer, [cc_no])

        return render_template('receipts_customer_by_zip.jinja2', customers = results)
